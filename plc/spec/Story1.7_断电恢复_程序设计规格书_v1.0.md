...
| POU | 类型 | 动作 | 职责 |
|---|---|---|---|
| **FC0 SysInit** | **功能块** | **★ v1.1 重构: 保留为调度骨架** | **冷启/断电判断 + Modbus清零 + FC21计数器清零 + 路径调度(CALL SBR25/CALL SBR26) + LBL 2 完成汇合** |
| **SBR25 ColdStart** | **子例程** | **★ v1.1 新增** | **冷启动路径完整逻辑: 出厂硬编码 + 用户默认覆盖(LBL 8/10) + TODR读RTC + RTC合法性检查(LBL 88/89) + 安全初始化 + RET** |
| **SBR26 WarmRecovery** | **子例程** | **★ v1.1 新增** | **断电恢复路径完整逻辑: TODR VB900 + RTC合法性检查 + DT10比较(LBL 3/4) + BCD转秒 + VD_S5_Elapsed重算 + 状态机恢复(LBL 5/6/7) + RET** |
| FC11/FC15/FC17/FC16 | 修改 | 补充注释 | 断电恢复状态保护说明(实际逻辑在FC0/SBR25/SBR26统一处理,状态FC无需改代码) |
