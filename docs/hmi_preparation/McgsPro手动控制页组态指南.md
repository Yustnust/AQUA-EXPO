...
| U1_DI_ValveA_Open ~ U1_DI_ValveC_Close | I8.0 ~ I8.5 | 阀门开/关到位反馈 |
...
├────────────────────────────────────────────────────────────────────────┤
│  ┌────────── 阀门控制 ──────────┐ ┌────────── 泵控制 ──────┐ ┌────── 注射泵手动控制 ──────┐ │
│  │ 阀A(上缸进水)                  │ │ 潜水泵1(上缸搅拌)       │ │ 总加药量: [      ] µL      │ │
│  │ 状态: ●关  反馈:关到位         │ │ 状态: ●停              │ │ 模式: [○单次 ○循环]        │ │
│  │ [手动开阀A]  [手动关阀A]       │ │ [手动启动] [手动停止]   │ │ 状态: 空闲                 │ │
│  │                                │ │                        │ │ [手动启动]  [手动停止/回零]│ │
│  │ 阀B(上→下转移)                │ │ 潜水泵2                 │ │ 注射泵状态: 0 位置: 0      │ │
│  │ 状态: ●关  反馈:关到位         │ │ 状态: ●停              │ │                            │ │
...
SELECT CASE Manual_Pending_Cmd
    ...
    CASE "SyringePump_Start"
        U1_CMD_Manual_SyringePump_Start = 1
    CASE "SyringePump_Stop"
        U1_CMD_Manual_SyringePump_Stop = 1
END SELECT
...
注射泵手动控制由新增 **FC21_ManualSyringePump** 实现，OB1 仅在 S0 态调用。
...
| VD_StepResolution | VD350 | 注射泵单步分辨率，用于步数计算 |
...
- 停止按钮不是急停，只是置位"停止请求"。
- 当前吸液/排液周期仍会完成，完成后进入回零，然后结束。
- 运行中再次按启动按钮无效，防止重置当前周期。

### 9.5 安全与互锁

- 仅 S0 态可用；非 S0 态 FC21 不被调用，内部状态自动清零。
- 注射泵通讯失败（V303.4）或状态码错误（VW4≥4）时，立即停止并置 VW390=99。
- 手动模式使用独立的累计/剩余变量（VD392/VD396），不影响自动 S3 的 VD370/VD372/VD378。

## 10. 已知限制与后续

- **NC 球阀串联主路**：手动开阀 A 时 PLC 自动打开上缸 NC 阀（Q0.5），开阀 B 时自动打开下缸 NC 阀（Q0.6），关阀后自动关闭。HMI 无需单独控制 NC 阀，但可添加状态指示灯观察 Q0.5/Q0.6。
- **注射泵手动抽/排液**：本期通过 FC21 实现启动/停止/单次/循环控制，参数为 VD384（总加药量）和 VW388（模式）。
