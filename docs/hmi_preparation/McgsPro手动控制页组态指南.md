...
| U1_潜水泵1 / U1_潜水泵2 | I0.3 / I0.4 | 与水泵串联的流量开关信号，直接反映水泵是否正常运行 |
...
| 泵1 运行/流量 | 指示灯 | U1_潜水泵1 | R | I0.3，与水泵串联的流量开关 |
| 手动启/停泵1 | 标准按钮/位按钮 | U1_CMD_Manual_Pump1_On/Off | W | 二次确认 + 脉冲 |
| 泵2 运行/流量 | 指示灯 | U1_潜水泵2 | R | I0.4 |
...
| 10 | 点击手动关阀A，V2.5 被置 1，阀A 关闭 | 阀A 关到位反馈变 ON |
| 11 | 手动启停泵1，I0.3（U1_潜水泵1）状态相应变化 | 泵运行/流量指示灯变化 |
| 12 | 8 个单元切换时，变量绑定随 `SelectedUnit` 切换 | 每个单元独立操作 |

---

## 9. 已知限制与后续

- **NC 电磁阀串联主路**：手动开阀 A 时 PLC 自动打开上缸 NC 阀（Q0.5），开阀 B/C 时自动打开下缸 NC 阀（Q0.6），关阀后自动关闭。HMI 无需单独控制 NC 阀，但可添加状态指示灯观察 Q0.5/Q0.6。
