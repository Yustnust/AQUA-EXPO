...
| U1_DI_FlowSwitch_A/B | I0.0 ~ I0.1 | 阀 A/B 流量开关状态；I0.2 保留未使用 |
...
- 手动模式使用独立的累计/剩余变量（VD392/VD396），不影响自动 S3 的 VD584/VD372/VD378。

## 10. 已知限制与后续

- **NC 球阀串联主路**：手动开阀 A 时 PLC 自动打开上缸 NC 阀（Q0.5），关阀后自动关闭。**v10.4 硬件变更：下缸 NC 球阀 Q0.6 已去除，手动开阀 B 不再联动 NC 阀**。HMI 可保留 Q0.5 状态指示灯，Q0.6 保留未使用。
