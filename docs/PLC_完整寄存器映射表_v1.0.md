...
**v2.2 PLC 代码实施前必须先完成以下 3 项迁移**,详细方案见《v2.2_实施前置迁移方案_v1.0.md》。本节给出迁移总览:

**1️⃣ V304.0 / V304.1 / V304.2 → M16.4 / M16.5 / M16.6**(位对象,无字节级冲突风险)
- 改 FC0_SysInit.stl L58/L63/L78(共3处 V304.0 引用 + 注释 L17)
- 改 FC4_ModbusPolling.stl RTU 在线标志写入处(V304.1/2)
- 改 OB1_MAIN.stl L134(VB305 与 VW304 高字节重叠需互锁处理)
- STEP 7 系统块:配置 M16.0\~M16.7 断电保持属性

**2️⃣ VW414 → VW236**(4 字节对齐真空闲区)
- 改 FC11_State_S1_Inlet.stl L85(MOVW T37, VW414 → VW236)
- 改 FC15_State_S4_Transfer.stl L123(MOVW T40, VW414 → VW236)
- 改 FC17_State_S6_Drain.stl L53(MOVW T42, VW414 → VW236)
- ITD/DTR 目标 VD444 不变(VW236 仅作暂存中转)

**3️⃣ VD442 / VD446 → VD256 / VD244**(4 字节对齐真空闲区)
- 改 FC40_RhythmCorrection.stl 整体重写为双倒计时器版(参见 v2.2 方案第七章 + PLC 设计文档第十章·四节)
- NETWORK 6 中双倒计时器显示值写入目标地址:VD442/VD446 → VD256/VD244

**验证方法**:实施每个阶段后,运行以下 grep 确认目标地址未冲突:
```bash
grep -nE "M16\\.[456]|VW236|VD256|VD244" plc/stl/*.stl
```

**预期输出**:0 命中(目标地址真空闲)。

**回退方案**:若实施过程中发现冲突或异常,按《v2.2_实施前置迁移方案_v1.0.md》第九章"回退方案"操作。
...
**已使用范围**: M10.0 \~ M15.4（共 45 个位，6 个字节 MB10\~MB15）**
| 地址           | �?    | 类型  | 引用FC                       | 用�?                         | 是否冲突   |
| ------------ | ----- | --- | -------------------------- | --------------------------- | ------ |
...
| I0.2 | 保留未使用（原流量开关C） | v10.4 硬件变更：I0.2 保留未使用，程序不再读取 | — |
...
| Q0.6       | 保留未使用（原下缸NC球阀） | v10.4 硬件变更：Q0.6 保留未使用 | — | — | — |
