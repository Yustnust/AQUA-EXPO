# PLC 完整寄存器映射表（所有区）
> **文档性质**: 机器生成 + 人工确认。由 Python 静态分析器扫描所有 STL 文件 + MCGS CSV 自动生成。> **生成日期**: 2026-09-13 (v1.1 更新：VD370→VD584 迁移、FC0 的 POU 拆分、冷启动 TODR + RTC 检查新增
...
## 一、字节级占用全景（VB0 \~ VB908）
颜色图例：🟢正常独占 🟡 多FC共用但不冲突 🔴 物理重叠/多用途冲突 🔵 HMI参数 ⚠️空闲
...
| VB 60\~VB 63 | 位区 V60.0~V63.7 (32个) | FC11/13/15/17/19/1A/2/3/30/31/40/21 | EU缓存位(M10.7/M16.x/V62.x/V63.x等) | **【2026-09-24定案】EU缓存位区**: 40+个线圈每扫描无条件写入,**禁止按REAL/WORD分配**;原VD60(排液完成验证延时)已迁VD472 |
...
| VB460\~VB523 | REAL/DWORD (4B) | FC0, MCGS | U1\_UD\_VD24\_ExpTarget\@460 \~ U1\_UD\_VD370\_VolTarget\@520 | **用户默认存储区**: VD460\~VD520 �6个REAL, �8个工艺参数的用户默认� | � | � |
| VB472\~VB475 | REAL (4B) | FC17,SBR25,SBR26 | U1\_VD\_Delay\_ValveC\_Verify\@472 | **VD\_Delay\_ValveC\_Verify 排液完成验证延时**(默认5.0s,HMI读写)【2026-09-24由VD60迁入,四维占用核实通过】 |
...
| VB550\~VB551 | WORD (2B) | FC0+SBR25+SBR26,FC21 | <br /> | FC21监控计数器新位置(上电MOVW 0清零,运行INCW累加) | � | � |
...
| VB584\~VB587 | REAL/DWORD (4B) | FC0\_SBR25,FC13,MCGS | U1\_VD\_Vol\_Target\@584 | **VD\_Vol\_Target 目标加药量(µL), 原VD370迁来, 断电保持(与VB414\~VB583连续) | 🔵 HMI参数 | �� |
| VB588\~VB591 | REAL/DWORD (4B) | - | <br /> | 空闲 | � | � |
| VB592\~VB599 | - | - | <br /> | 空闲（VB550\~VB599 合计50B, FC21计数器17个34B + VD584(4B) + �2B� | � | � |
...
**根因**(2026-09-07 VD 参数区重排时，浓度参数先被分配到 VD10/VD14，后来有人加了 DT10（FC15 S4 完成�RTC），选地址时没查冲突。两者物理地址完全重叠。
**破坏**�
1. 冷启动时 FC0 的 `MOVR 5.0, VD10` / `MOVR 1000.0, VD14` 初始化浓度。2. HMI 运行后用户可设置浓度。3. 状态机到 S4 且下缸满时 FC15 写 `DT10 = READ_RTC()` �**VB10\~VB17 全被 RTC BCD 覆盖**。4.. FC13 S3 加药时读 `VD10 × VD90 / VD14` �**分子分母�RTC 时间戳的垃圾�* �5. 但冷启动又重初始化了 �现场没暴露，�*隐藏炸弹**

**修复方案（已对齐 2026-09-12）**�
1. **删除 VD10 目标浓度** �**VD14 母液浓度**（HMI 参数，简化为用户直接输入加药量）
2. 但**DT10 RTC 原地保留**（VB10\~VB17 8 字节现在完全独占，安全了！）
...
3. VD382 的 IEEE-754 浮点指数位（bit30\~23 即VB382 的高 7 位）被清零，值变成**4.23E-37**
...
| -------------------------------- | ----- | --------- | ------------ | --------- |
| VD\_Dosed\_Volume\_Total 累计加药量 | VD378 | **VD440** | VB440\~VB443 | � |
...
> **为什么不复用 VD584**：VD584 是VD\_Vol\_Target（FC13 S3 用的目标加药量），虽然S3结束后到下一个S3是空窗，但不留隐患。全部迁到空闲区。
> **为什么不碰MBUS Error**：你明确要求MBUS Error 原地独占，FC4 的代码一行不动。
...
**破坏**：状态机每次到S4时计算T61 PT 值，**顺便覆盖了VD384**（ManualDose\_Target）。然�FC21 启动手动注射泵时 `MOVR VD384, VD396` 取"剩余药量"，**拿到的是 VD382×10 = 18000.0 这种垃圾**。�
...
| 冲突 | 原地址 | 修复日期 | 迁移到 |
| ----------------------------- | ------------ | -------------------- | -------------- |
| 手动控制命令位V2.4\~V3.7 vs VW2 状态机 | VB2\~VB3 | 2026-09-03 (v1.8) | V306.0\~V307.3 |
| VW250/VW252 vs HMI 参数 VD250 | VB250\~VB253 | 2026-07-18 (AQEX-36) | VW290\~VB296 |

***

### "架构性质冲突"（合理设计，无需修复）
下列字节在Python 静态分析中被标记，但经人工核实为**有意设计**。
| 区域 | 说明 |
| ------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
| VB364\~VB365 | VD362(4B) 的高 2 字节覆盖 VD364(4B) 的低 2 字节。但 VD362(ValveB Timeout) 是**秒级整数**（范围30\~300），�WORD 范围内，低2字节恒=0。VD364(ExpTotal\_Flow) �0 开始每分钟累加，低 2 字节 = 小数部分（L）。两�*低字节互不干�*�|
...
| VB414 | VB499 | 86 | 21 | ⚠️ 非对齐 | ⭐ 大区域，适合批量参数; **VB472~475已分给VD\_Delay\_ValveC\_Verify**(2026-09-24) |
...
| 区域 | 最大容量 | 当前已用 | 备注 |
| ----- | --------------------- | -------- | ----------- |
| V 区 | 1024 字节 (VB0\~VB1023) | \~890 字节 | 已用 87%，接近上限 |
...
## 九、2026-09-24~25 联机调试变更记录（v1.2）

### 9.1 问题清单与修复

| # | 现象 | 根因 | 修复 |
|---|------|------|------|
| 1 | HMI正常状态显示"急停" | OB1 NETWORK 10 急停显示逻辑未随 v1.3 NO接法更新(仍NOT I1.1) | OB1 已修 |
| 2 | 未点启动上缸自启且跳过S1 | FC19系统复位路径不清VW304;OB1上缸调度无VW2门控 | FC19/OB1 已修 |
| 3 | VD60写入无效/闪变/NaN | VD60与EU缓存位区V60.x~V63.x重叠,40+线圈每扫描撕碎浮点 | VD60废弃,迁VD472(FC17/SBR25/SBR26) |
| 4 | 排液完成验证延时出厂默认0 | HMI从未设置过默认参数,出厂值责任在HMI侧 | HMI侧治本:出厂设置兜底(已定案) |
| 5 | 每轮末必报"单轮换水周期超时" | FC40 NET3在S6排水期继续计数+NET6门控缺陷(VD116=0同阈值竞争) | FC40已修:排水期冻结计数,报警改"错过窗口才报"(2026-09-25) |
| 6 | 报警66确认无效、锁存到死 | 全程序无R V303.6路径;OB1中FC3先于FC40,只清位会被同扫描重报(死锁) | FC3 NETWORK4补复位+双计时器清零(确认=本轮重新计时)(2026-09-25) |

### 9.2 关键决策

- **VD60→VD472迁移**：VD60(VB60~63)与EU缓存位区物理重叠，永久废弃；镜像变量名`U1_UD_VD60_DelayC`(VD492)保留不改。
- **HMI出厂默认兜底**：客户未设置过默认参数时采用出厂设置，防止出现0；ColdStart恢复为无条件覆盖，不加门控。
- **报警66语义**："错过S6触发窗口才报"；确认=清位+VD248/252清零(重新计时)，节奏自动恢复。
- **下装SOP**：新程序下装后必须清除V存储器再上电（NaN自愈依赖）。

### 9.3 现场待办

| # | 事项 | 状态 |
|---|------|------|
| 1 | McgsPro通道U1~U8_VD_Delay_ValveC_Verify地址 VDF060→VDF472 | 单元2~8 CSV已改；**单元1 CSV被占用未改**，关闭McgsPro后补改 |
| 2 | 下装块清单(9块)：OB1/FC3/FC3A/FC4/FC17/FC19/FC40/SBR25/SBR26 | 待下装 |
| 3 | 调试SOP补"清V区再上电"步骤 | 待落实 |
| 4 | 核对M区断电保持范围是否含MB16 | 待核对 |
| 5 | HMI参数页脚本加NaN防护 | 待落实 |
| 6 | HMI脚本库与现场漂移核对(S4WaitTimeout *60等) | 待核对 |

### 9.4 教训登记

- EU缓存位区(V60.x~V63.x)按位分配后，**禁止**再按REAL/WORD分配给任何变量；新变量先查映射表+静态分析器。
- 报警位设计必须成对考虑置位/复位路径；FC40注释承诺"V0.3人工确认"但复位线缺失,锁存到死。
- 用脚本批量修复文档/编码前必须先备份(git提交),本次映射表乱码即历史修复脚本造成,git无干净版时按字节模式恢复成本极高。
- OB1内FC3先于FC40执行,跨FC的"置位-复位"时序必须画扫描序图核对,否则确认键死锁。
## 十、版本历史
| 版本 | 日期 | 变更 |
| ---- | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| v1.2 | 2026-09-24~25 | **联机调试变更**:(a)VD60与EU缓存位区冲突,废弃迁VD472 (b)FC40周期超时报警改"错过窗口才报"+S6排水期冻结计数 (c)FC3补V303.6确认复位+计时器清零 (d)OB1急停显示/FC19复位路径/FC4残留复位修复 (e)HMI出厂默认参数兜底定案 |
| v1.1 | 2026-09-13 | **FC21 计数器迁出**：（a）根因定位：FC21 调试计数器VW516~~VW548 覆盖用户默认存储区VB456~~VB539（VD516\~VD528/VW532/V536.0），导致 FC0 上电 MOVW 0 清零用户默认值，断电后再上电即丢失（b）修复：VW516~~VW548 全部迁到 VB550~~VB583，FC0 清零指令�FC21 INCW 同步修改（c）补齐映射表漏掉�VB416~~VB548 区域：新增用户默认存储区 VB456~~VB539 完整条目，标记空闲区 VB416~~VB455 / VB540~~VB548（d）更�L301 报警日志缓冲描述：移�+ FC21 监控计数�，标记已迁出 |
