...
## v2.1 现场实施 SOP (2026-09-12)

### 断电保持范围配置（必须！否则用户默认参数断电丢失）

STEP7-MicroWIN SMART:
1. 左侧树 → CPU (SR40) → **系统块** → 双击
2. 切换到 **断电保持** 选项卡
3. 范围 0: 起始地址 `VB0`, 元素数目 `540`
4. 点 **确认** → 编译 → 下载系统块到 PLC

```
范围0: V    VB0    ~    VB539     保持数量 540
```

**为什么是 VB0~VB539 而不是分开配？**
- 之前分 3 段配 (VB350~373 + VB448~455 + VB456~539), 容易遗漏或配错地址
- 统一连续 540 字节最安全, 涵盖所有需要断电保持的区域
- 不包括 VB540~VB899: 中间继电器/工作区, 不需要保持
- 不包括 VB900~VB907: RTC TODR 缓冲区, 每次冷启动 FC0 L312 TODR VB900 会重写

### RTC 自动校时 (方案C)

MCGS 脚本 55 已升级为全自动:
- PLC 冷启动 RTC 丢失 → V303.7=1
- MCGS 循环策略 1秒内自动取本机时间 → BCD → VB900~VB905 → U1_CMD_RTC_Sync=1
- PLC FC22 检测到 V0.6 上升沿 → TODW VB900 → RTC 恢复
- 弹窗 RTC_Sync_Wnd 作为轻提示 (非阻塞)
- 脚本56 手动校时按钮保留作为兜底

**工控机 NTP 设置建议**:
- Windows 设置 → 时间和语言 → 日期和时间 → 打开 "自动设置时间"
- 如果不能联网, 定期人工巡检 Windows 系统时间

### 用户默认参数存储 (U1_UD_*)

| 地址 | 长度 | 用途 |
|------|------|------|
| VB456 | 1B | 用户默认有效标志 (0=出厂, 1=用户) |
| VB457~459 | 3B | 预留对齐 |
| VD460~VD528 | 72B | 18 个 REAL 用户默认值 |
| VB532~VB533 | 2B | VW388 (ManualDose_Mode) |
| VB534~VB535 | 2B | 预留 |
| VB536 | 1B | V200.0 (AlarmAckMode bit0) |
| VB537~VB539 | 3B | 预留 |
...
| FC | 改动 |
|----|------|
| FC0 | 出厂硬编码 + 用户默认覆盖逻辑 (LBL 8/9/10) |
| FC15 | TODR VB900 → DT10 (L130~138, S4 完成时记录时间戳) |
| FC22 | RTC 自动校时 (TODW + 5分钟超时保护) |
