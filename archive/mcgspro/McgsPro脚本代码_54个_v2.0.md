...
**用途**: 直接粘贴到 McgsPro 脚本编辑器,完成4菜单窗口+9画面+策略+子窗口的脚本组态
**Story**: AQEX-12 Story 2.1 (8画面组态) — v2.0 重写为纯正 McgsPro 类Basic语法 — v2.1 新增两级菜单导航脚本

> **v2.1 更新**: 新增两级菜单导航(主菜单4标签→子菜单→画面),新增9个菜单窗口脚本,所有返回按钮目标从画面1_总览改为各自上级菜单窗口,画面8拆分为画面8a_单元使能和画面8b_权限管理。详见《McgsPro两级菜单改造方案_v1.0》。
...
> **菜单导航原理**:主菜单4个标签→子菜单→画面。所有窗口切换使用 `!CloseAllWindow("窗口名")` (关闭除指定外所有窗口,若指定窗口未打开则同时打开它)。`CurrentMenuGroup` 变量记录当前所在菜单组(0=主菜单/1=单元操作/2=监控诊断/3=系统)。

### 脚本 M1:主菜单 → 总览按钮

- **编号**: M1
- **用途**: 从主菜单进入画面1_总览
- **位置**: 用户窗口 → 菜单_主菜单 → btnOverview 构件 → Click 事件
- **触发方式**: 按钮单击
...
- **编号**: M2
- **用途**: 从主菜单进入菜单_单元操作子菜单
- **位置**: 用户窗口 → 菜单_主菜单 → btnUnitOps 构件 → Click 事件
- **触发方式**: 按钮单击
...
- **编号**: M3
- **用途**: 从主菜单进入菜单_监控诊断子菜单
- **位置**: 用户窗口 → 菜单_主菜单 → btnDiag 构件 → Click 事件
- **触发方式**: 按钮单击
...
- **编号**: M4
- **用途**: 从主菜单进入菜单_系统子菜单
- **位置**: 用户窗口 → 菜单_主菜单 → btnSystem 构件 → Click 事件
- **触发方式**: 按钮单击
...
- **编号**: 17
- **用途**: 关闭所有子窗口,返回菜单_单元操作(v2.1改为两级菜单)
...
- **编号**: 26
- **用途**: 关闭手动控制画面,返回菜单_单元操作(v2.1改为两级菜单)
...
Param_Confirm_Text = "将参数设置应用于" + "1号单元的运行，确认保存？"
!OpenSubWnd(用户窗口.参数设置二次确认, 400, 300, 400, 200, 0)
```
...
- **编号**: 32
- **用途**: 关闭参数设置画面,返回菜单_单元操作(v2.1改为两级菜单)
...
> **v2.1 拆分说明**:原"画面8_系统设置"拆分为两个独立画面:画面8a_单元使能(单元使能开关+状态指示)和画面8b_权限管理(登录/注销/修改密码/退出系统)。两画面均返回菜单_系统。

### 脚本 44:画面8a 窗口打开脚本

- **编号**: 44
- **用途**: 加载8个单元使能状态(v2.1从画面8拆出)
...
- **位置**: 用户窗口 → 画面8a_单元使能 → 单元使能开关构件(8个) → Click 事件
- **触发方式**: 开关单击
- **扩展方法**: 8个开关独立脚本,以1号为例
...
- **位置**: 用户窗口 → 画面8b_权限管理 → 退出系统按钮构件 → Click 事件
- **触发方式**: 按钮单击
...
- **编号**: 49
- **用途**: 关闭画面8b_权限管理,返回菜单_系统(v2.1改为两级菜单)
...
> **注**:画面8a_单元使能的返回按钮使用相同逻辑,脚本为 `!CloseAllSubWnd()` + `!CloseAllWindow("菜单_系统")`。
...
| 打开窗口 | `!SwitchWindow("画面2")` (不存在) | `!CloseAllWindow("画面2_单元详情")` |
