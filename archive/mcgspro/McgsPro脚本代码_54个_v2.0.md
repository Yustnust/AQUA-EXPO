...
**配套文档**: McgsPro变量导入_单元1.csv ~ 单元8.csv / HMI用户权限矩阵_v1.0.md / HMI画面架构规划文档.md / McgsPro两级菜单改造方案_v1.0.html
**用�?*: 直接粘贴�?McgsPro 脚本编辑�?完成4菜单窗口+9画面+策略+子窗口的脚本组�?
**Story**: AQEX-12 Story 2.1 (8画面组�? �?v2.0 重写为纯�?McgsPro 类Basic语法 �?v2.1 新增两级菜单导航脚本

> **v2.1 更新**: 新增两级菜单导航(主菜�?标签→子菜单→画�?,新增9个菜单窗口脚�?所有返回按钮目标从画面1_总览改为各自上级菜单窗口,画面8拆分为画�?a_单元使能和画�?b_权限管理。详见《McgsPro两级菜单改造方案_v1.0》�?
...
| `Sub ... End Sub` / `Function ... End Function` | McgsPro 不支持自定义子程�?子函�?所有逻辑必须内联 |
| `ElseIf ... Then` | 用嵌�?`If...Then...Else...EndIf` 实现多分�?|
| `Execute("U" & i & "_CMD_Mute = 1")` | **无法动态构造变量名**,必须显式写出 8 个单元的赋值语�?|
| `MsgBox(...)` / `InputBox(...)` | 用子窗口(!OpenSubWnd)实现确认/选择对话�?|
| `!OpenWindow` / `!SwitchWindow` | �?`!SetWindow(窗口,1)` 打开、`!CloseAllWindow(name)` 关闭并切�?|
...
| `Dim x` (无类�? | 必须�?`DIM x AS integer` (4 种类�? byte/integer/single/string) |
...
| `!CloseSubWnd(窗口对象)` | 关闭指定子窗�?|
| `!CloseAllWindow(WndName)` | 关闭所有窗�?WndName 非空则保留并打开�? |
| `!SetWindow(窗口对象,Op)` | Op=1打开可见/2打开不可�?3关闭/4打印/5刷新 |
...
| `!EnableExitLogon(n)` | n=0不检�?1检�?2超时提示/3超时静默 |
...
本工�?8 套单元变量命名规�?`U<单元�?_XXX`,�?`U1_CMD_Start` ~ `U8_CMD_Start`。由�?McgsPro **不支�?Execute 动态构造变量名**,凡涉�?�?8 个单元循环操�?的脚�?本文件采�?*显式展开 8 条赋值语�?*的写�?而非 For 循环。涉�?�?1 号为�?的相似脚�?会给�?1 号完整代�?+ 2~8 号扩展说明�?
...
> 说明:v2.1 新增 B2 �?菜单窗口脚本 M1~M9,�?�?,含主菜单4个按钮脚本�?个子菜单的画面按钮脚本�?个子菜单的返回按钮脚�?返回逻辑相同合并�?�?。原 J �?画面8_系统设置"拆分为画�?a_单元使能(脚本44~45)和画�?b_权限管理(脚本46~49)。合�?57+9=66 个�?
...
- **编号**: 1
- **用�?*: 工程启动时初始化内部变量、启动所�?PLC 设备、加载用户配�?
- **位置**: 运行策略 �?启动策略 (系统固有策略�?
- **触发方式**: 系统启动时自动执行一�?
...
' --- 3. 初始化单元使�?从断电保持区读取,首次启动默认1号使�? ---
' U1_Enable~U8_Enable 已配置为断电保持内部变量,此处仅在值为0时给默认�?
If U1_Enable = 0 Then
    If U2_Enable = 0 Then
        If U3_Enable = 0 Then
            If U4_Enable = 0 Then
...
- **编号**: 2
- **用�?*: 周期清零命令�?模拟上升�? + 更新通讯在线状�?+ 更新 CommStatus 汇�?
...
' ============================================
' 500ms 周期策略
' 功能1: 清零8个单元的命令�?模拟脉冲上升�?PLC侧用下降沿检�?
' 功能2: 检�?台PLC通讯状�?更新 U1_Online~U8_Online �?CommStatus
' 注意: McgsPro 不支�?For...Next,8 个单元显式展开
' ============================================
...
' ============================================
' 1秒周期策�?
' 功能1: 拼接 $Date + $Time 写入 SysTimeString (供画面顶部显�?
' 功能2: 检查登录超�?15分钟无操作自动注销)
' ============================================
...
' --- 3. 检�?EnableExitLogon 超时模式(2=超时提示,3=超时静默) ---
' �?!EnableExitLogon 内部处理,此处仅记录最近操作时�?
...
> **菜单导航原理**:主菜�?个标签→子菜单→画面。所有窗口切换使�?`!CloseAllWindow("窗口�?)` (关闭除指定外所有窗�?若指定窗口未打开则同时打开�?。`CurrentMenuGroup` 变量记录当前所在菜单组(0=主菜�?1=单元操作/2=监控诊断/3=系统)�?
...
### 脚本 M5:菜单_单元操作 �?画面按钮(详情/手动/参数)

- **编号**: M5
- **用�?*: 从子菜单进入对应业务画面
- **位置**: 用户窗口 �?菜单_单元操作 �?btnDetail / btnManual / btnParam 构件 �?Click 事件
- **触发方式**: 按钮单击
...
### 脚本 M8:子菜单返回主菜单(菜单_单元操作/监控诊断/系统通用)

- **编号**: M8
- **用�?*: 从任意子菜单返回主菜�?
- **位置**: 用户窗口 �?菜单_单元操作 / 菜单_监控诊断 / 菜单_系统 �?btnBack 构件 �?Click 事件(3个子菜单共用相同脚本)
- **触发方式**: 按钮单击
...
' --- 4. 刷新全局报警/消音状�?�?00ms周期策略持续更新,此处只读) ---
...
- **编号**: 5
- **用�?*: 点击1号单元卡�?设置 SelectedUnit=1,打开画面2_单元详情子窗�?
- **位置**: 用户窗口 �?画面1_总览 �?1号单元卡片构�?�?Click 事件
- **触发方式**: 卡片鼠标单击
...
- **编号**: 6
- **用�?*: 2~8号单元卡片点�?逻辑同脚�?,仅单元号不同
- **位置**: 用户窗口 �?画面1_总览 �?2~8号单元卡片构�?�?Click 事件(7个独立脚�?
- **触发方式**: 卡片鼠标单击
- **扩展方法**: 将脚�?中的 `U1_Enable` / `U1_Online` / `SelectedUnit = 1` 替换为对应单元号
...
' ============================================
' 全局消音按钮脚本
' 功能: 对所有使�?在线单元�?CMD_Mute=1 (500ms周期策略会自动清�?
' ============================================
...
- **编号**: 9
- **用�?*: 急停按钮,弹出二次确认子窗�?确认后对8个单元下�?CMD_Stop=1
...
' --- 2. 打开急停确认子窗�?模�?边框,居中位置 240,180,400,180) ---
...
- **编号**: 10
- **用�?*: 根据 SelectedUnit 加载对应单元的数�?显示"当前操作:X号单�?
...
' ============================================
' 报警确认按钮脚本(以当前选中单元为例)
' 功能: �?SelectedUnit 对应单元�?CMD_AckAlarm=1
' ============================================
...
' ============================================
' 消音按钮脚本
' 功能: �?SelectedUnit 对应单元�?CMD_Mute=1
' ============================================
...
- **编号**: 17
- **用�?*: 关闭所有子窗口,返回菜单_单元操作(v2.1改为两级菜单)
...
If SelectedUnit = 1 Then
    CurrentUnitStr = "当前操作:1号单�?(手动模式)"
EndIf
If SelectedUnit = 2 Then
    CurrentUnitStr = "当前操作:2号单�?(手动模式)"
EndIf
If SelectedUnit = 3 Then
    CurrentUnitStr = "当前操作:3号单�?(手动模式)"
EndIf
If SelectedUnit = 4 Then
    CurrentUnitStr = "当前操作:4号单�?(手动模式)"
EndIf
If SelectedUnit = 5 Then
    CurrentUnitStr = "当前操作:5号单�?(手动模式)"
EndIf
If SelectedUnit = 6 Then
    CurrentUnitStr = "当前操作:6号单�?(手动模式)"
EndIf
If SelectedUnit = 7 Then
    CurrentUnitStr = "当前操作:7号单�?(手动模式)"
EndIf
If SelectedUnit = 8 Then
    CurrentUnitStr = "当前操作:8号单�?(手动模式)"
EndIf
```
...
- **编号**: 19
- **用�?*: 手动开/关阀A(需维护组权�?。开按钮�?CMD_Manual_ValveA_Open=1,关按钮置 CMD_Manual_ValveA_Close=1
- **位置**: 用户窗口 �?画面3_手动控制 �?阀A开按钮 / 阀A关按�?�?Click 事件
- **触发方式**: 按钮单击
...
- **编号**: 20
- **用�?*: 手动开/关阀B(需维护组权�?
- **位置**: 用户窗口 �?画面3_手动控制 �?阀B开按钮 / 阀B关按�?�?Click 事件
- **触发方式**: 按钮单击
...
- **编号**: 21
- **用�?*: 手动开/关阀C(需维护组权�?
- **位置**: 用户窗口 �?画面3_手动控制 �?阀C开按钮 / 阀C关按�?�?Click 事件
- **触发方式**: 按钮单击
...
- **编号**: 22
- **用�?*: 手动开/关潜水泵1(需维护组权�?
- **位置**: 用户窗口 �?画面3_手动控制 �?�?开按钮 / �?关按�?�?Click 事件
- **触发方式**: 按钮单击
...
' ============================================
' 复位按钮脚本
' 功能: 校验管理员组权限 �?对选中单元�?MB_Pump_Reset=1
' ============================================
...
- **编号**: 26
- **用�?*: 关闭手动控制画面,返回菜单_单元操作(v2.1改为两级菜单)
...
' ============================================
' 画面4_参数设置 Load 脚本
' 功能: �?SelectedUnit 对应单元�?VD 参数读到编辑缓冲变量
'       (供输入框编辑,保存时由脚本28写回 PLC)
' 注意: 浓度组参�?VD10/VD14)仅管理员可见可改,这里统一加载,
'       组态时通过权限位隐藏浓度输入框
' ============================================
...
- **编号**: 28
- **用�?*: 校验参数范围 �?写入选中单元�?VD 参数(需维护组权�?浓度组需管理�?
...
- **编号**: 29
- **用�?*: 弹出单元选择子窗�?选择目标单元后复制参�?需管理员组权限)
- **位置**: 用户窗口 �?画面4_参数设置 �?复制到其他单元按钮构�?�?Click 事件
- **触发方式**: 按钮单击

```
' ============================================
' 复制到其他单元按钮脚�?
' 功能: 权限校验 �?弹出单元选择子窗�?脚本56负责选目标单�?
'       脚本57负责确认后执行复�?
' ============================================
...
' 默认值清�?参�?McgsPro变量导入_单元1.csv ~ 单元8.csv 备注):
...
- **触发方式**: 按钮单击
- **前置**: MCGS 需先导�?McgsPro变量导入_单元1_v3.0.csv (新增 21 �?U1_UD_ 变量)
...
IF Param_RestTime_Min > Param_RestTime THEN
    Param_Confirm_Text = "错误：静止等候压缩下限不得大于标称时�?
    Param_Pending_Save = 0
    !OpenSubWnd(用户窗口.保存默认二次确认, 400, 300, 400, 200, 0)
    EXIT
ENDIF
...
IF Param_CycleExtend_Max > Param_CycleSet THEN
    Param_Confirm_Text = "错误：换水周期顺延上限不得大于换水周�?
    Param_Pending_Save = 0
    !OpenSubWnd(用户窗口.保存默认二次确认, 400, 300, 400, 200, 0)
    EXIT
ENDIF
...
IF Param_T_Default < 1 OR Param_T_Default > 600 THEN
    Param_Confirm_Text = "错误：首轮配液总时长T超范�?1~600min)"
    Param_Pending_Save = 0
    !OpenSubWnd(用户窗口.保存默认二次确认, 400, 300, 400, 200, 0)
    EXIT
ENDIF
...
IF Param_S6_Default < 1 OR Param_S6_Default > 600 THEN
    Param_Confirm_Text = "错误：首轮S6排水时长超范�?1~600min)"
    Param_Pending_Save = 0
    !OpenSubWnd(用户窗口.保存默认二次确认, 400, 300, 400, 200, 0)
    EXIT
ENDIF
...
- **编号**: 31
- **用�?*: 完整校验时间/超时/工艺参数上下�?失败时蜂�?退�?
- **位置**: 用户窗口 �?画面4_参数设置 �?输入框构�?�?ContentChanged 事件(或保存前调用)
...
'   预循�? PreMixTime_MinSafe(3.0) <= PreMixTime <= 60
...
EndIf
    ParamValid = 0
    ParamInvalidStr = "�?超时下限0.5s"
EndIf
    ParamValid = 0
    ParamInvalidStr = "�?超时上限30s"
EndIf
    ParamValid = 0
    ParamInvalidStr = "�?超时下限0.5s"
EndIf
...
- **编号**: 32
- **用�?*: 关闭参数设置画面,返回菜单_单元操作(v2.1改为两级菜单)
...
- **编号**: 34
- **用�?*: 弹出单元选择子窗�?选择目标单元号筛选报�?
- **位置**: 用户窗口 �?画面5_报警日志 �?按单元筛选按钮构�?�?Click 事件
- **触发方式**: 按钮单击

```
' ============================================
' 按单元筛选按钮脚�?
' 功能: 打开单元选择子窗�?脚本56),用户选择后更�?AlmFilterUnit
' ============================================
...
- **触发方式**: 按钮单击
- **前置条件**: 画面上预先放3个历史曲线构�?分别对应1h/8h/24h,每个构件�?高级属性→时间范围"在属性对话框�?*静态配�?*(不能用变量绑�?
...
'
'   正确做法:
'   画面上预先放3个历史曲线构�?HisCurve_1h / HisCurve_8h / HisCurve_24h),
'   每个构件�?高级属性→时间范围"分别写死�?3600/28800/86400 秒�?
'   切换按钮仅改�?个构件的 Visible 绑定变量值�?
...
- **编号**: 41
- **用�?*: 权限校验后停止所�?PLC + Modbus 设备,延时 500ms 再全部启�?全量重启)
- **位置**: 用户窗口 �?画面7_通讯维护 �?重启通讯按钮构件 �?Click 事件
- **触发方式**: 按钮单击
- **二次确认**: 按钮安全属�?�?安全控制 �?弹窗确认(需在构件属性中配置,提示文本="将重启所有单元通讯连接,期间数据采集暂停,确认?")
- **权限**: 按钮安全属性配�?LoginLevel >= 3 �?!CheckUserGroup("管理员组")
- **设备命名约定**: McgsPro 设备窗口中设备名需与脚本一致。`PLC_01`~`PLC_08` �?S7-200 SMART 单元 PLC 设备,`MB_Pump_01`~`MB_Pump_08` �?8 台注射泵 Modbus 从站,`MB_Flow_01`~`MB_Flow_08` �?8 台流量计 Modbus 从站。如工程实际命名不同(�?`PLC_1` 无前导零),请同步修改脚�?

```
' ============================================
' 重启通讯按钮脚本
' 功能: 权限校验 �?停止所有PLC+Modbus设备 �?延时500ms �?启动所有设�?
' 注意: 全量重启会影�?套单元通讯,�?秒采集暂�?
' 二次确认由按钮安全属性弹窗实�?本脚本不做确�?
' ============================================
...
## 十、J. 画面8a_单元使能 + 画面8b_权限管理脚本

> **v2.1 拆分说明**:�?画面8_系统设置"拆分为两个独立画�?画面8a_单元使能(单元使能开�?状态指�?和画�?b_权限管理(登录/注销/修改密码/退出系�?。两画面均返回菜单_系统�?

### 脚本 44:画面8a 窗口打开脚本

- **编号**: 44
- **用�?*: 加载8个单元使能状�?v2.1从画�?拆出)
- **位置**: 用户窗口 �?画面8a_单元使能 �?Load 事件
- **触发方式**: 窗口装载�?

```
' ============================================
' 画面8a_单元使能 Load 脚本
' 功能: 关闭残留子窗�?+ 加载单元使能状�?从断电保持变�?
' ============================================
...
' --- 4. 拼接单元使能状态汇�?供画面显�? ---
' U1_Enable~U8_Enable 为断电保持内部变�?这里只读汇�?
UnitEnableSummary = ""
If U1_Enable = 1 Then
...
- **编号**: 45
- **用�?*: 切换单元使能状�?0�?) + !SaveData 存盘(需管理员组权限)
- **位置**: 用户窗口 �?画面8a_单元使能 �?单元使能开关构�?8�? �?Click 事件
- **触发方式**: 开关单�?
- **扩展方法**: 8个开关独立脚�?�?号为�?
...
**2~8号单元使能开关切�?*(扩展方法:把脚本中 `U1_Enable` 替换为对应单元号):
```
' 2�?
If !CheckUserGroup("管理员组") = 1 Then
    !Beep()
...
- **编号**: 49
- **用�?*: 关闭画面8b_权限管理,返回菜单_系统(v2.1改为两级菜单)
...
> **�?*:画面8a_单元使能的返回按钮使用相同逻辑,脚本�?`!CloseAllSubWnd()` + `!CloseAllWindow("菜单_系统")`�?

---

## 十一、K. 二次确认子窗口脚�?

> **子窗口组态约�?*:每个二次确认子窗口包�?确认"�?取消"两个按钮,确认按钮执行实际操作并调�?`!CloseAllSubWnd()` 关闭子窗�?取消按钮仅调�?`!CloseAllSubWnd()`。子窗口模式参数 `17 = 1(模�? + 16(边框)`�?

### 脚本 50:启动确认子窗�?

- **编号**: 50
- **用�?*: 启动二次确认。确认→�?CMD_Start=1 + 关闭;取消→关�?
...
- **编号**: 51
- **用�?*: 停止二次确认。确认→�?CMD_Stop=1 + 关闭;取消→关�?
...
- **编号**: 52
- **用�?*: 急停二次确认。确认→�?个使�?在线单元�?CMD_Stop=1 + 关闭;取消→关�?
...
' ============================================
' 急停确认子窗�?- 确认按钮
' 功能: 对所有使�?在线单元下发急停(CMD_Stop=1)
' 注意: 急停是全局操作,不限�?SelectedUnit
' ============================================
...
- **编号**: 53
- **用�?*: 恢复默认参数二次确认。确认→重置 Param_* 为默认�?+ 关闭;取消→关�?
...
- **编号**: 54
- **用�?*: 清除报警历史日志二次确认。确认→!ClearHistoryAlarmData + !OperationLogClear + 关闭;取消→关�?
...
- **编号**: 55
- **用�?*: 退出系统二次确认。确认→关闭所有窗�?退出运行环�?取消→关�?
...
- **编号**: 56
- **用�?*: 选择目标单元号�?个数字按�?1~8)+ 1�?全部"按钮,点击后写入目标单元变量并关闭
- **位置**: 用户窗口 �?子窗口_单元选择 �?1~8号单元按�?/ 全部按钮 �?Click 事件
- **触发方式**: 按钮单击
- **使用场景**: 脚本29(复制参数选择目标单元) / 脚本34(报警按单元筛�?
...
'   SubWndMode=1: 报警筛选单�?�?AlmFilterUnit
' ============================================
...
- **编号**: 57
- **用�?*: 跨单元复制参数二次确认。确认→�?ParamSrcUnit �?VD 参数复制�?ParamDstUnit + 关闭;取消→关�?
...
' ============================================
' 复制参数确认子窗�?- 确认按钮
' 功能: �?ParamSrcUnit 单元的全�?VD 参数复制�?ParamDstUnit 单元
' 前置条件: 脚本29已设�?ParamSrcUnit,脚本56已设�?ParamDstUnit
' 注意: McgsPro 不支持动态变量名,用嵌�?If 选择�?目标单元
' ============================================
...
'   - 内层 U1_VD_* (源读�? 全部替换�?U<src>_VD_*
'   - 内层 U<n>_VD_* (目标写入) 保持目标单元�?
' �? �?号→目标5号的关键�?
'   ... (其余18项同�?完整展开见组态实�?
' 完整脚本应在组态时�?8x7=56 种组合全部展开,本文件以�?号为代表�?
...
- `1` (bit0) = 模态模�?子窗口外鼠标不响�?必须 CloseSubWnd 关闭)
- `16` (bit4) = 显示边框
- 17 = 1 + 16

如需菜单模式(子窗口外按下鼠标自动关闭),改用 `2` �?`18` (2+16)。如需跟随鼠标位置弹出,�?`32` (bit5),例如 `49 = 1+16+32`�?
...
| 变量�?| 类型 | 用�?| 断电保持 |
|---|---|---|---|
| SelectedUnit | integer | 当前选中单元�?1~8 | �?|
| GlobalAlarmActive | integer | 全局报警激活标�?| �?|
| GlobalMuteState | integer | 全局消音状�?| �?|
| GlobalAckPending | integer | 全局待确认标�?| �?|
| CommStatus | integer | 在线单元计数 0~8 | �?|
| SysTimeString | string | 系统时间显示�?| �?|
| LoginTime | integer | 登录时间�?�? | �?|
| LastMouseTime | integer | 最后鼠标操作时�?| �?|
| ExitLogonEnabled | integer | 退出权限检查模�?| �?|
| U1_Online ~ U8_Online | integer | 8单元通讯在线状�?| �?|
| U1_Enable ~ U8_Enable | integer | 8单元使能配置 | **�?* |
| CurrentMenuGroup | integer | 当前菜单�?0=主菜�?1=单元操作/2=监控诊断/3=系统) | **�?* |
...
| TrendDisplayUnit | integer | 趋势显示单元 | �?|
...
| UnitEnableSummary | string | 使能单元汇总串 | �?|
...
`!CheckUserGroup("维护�?) = 0` 表示当前用户属于维护�?或更高权限组,如果维护组是管理员组的子�?。本文件采用"维护�?OR 管理员组"嵌套 If 实现向下兼容�?

### 4. 组对象存盘配�?

脚本45�?`!SaveData(UnitEnableGroup)` 要求:
- 在实时数据库创建组对�?`UnitEnableGroup`
- �?U1_Enable~U8_Enable 加入该组对象
- 勾�?定时存储到磁�?+ 存盘周期
...
- 子窗口_恢复默认确认 / 子窗口_清除日志确认 / 子窗口_退出系统确�?
- 子窗口_单元选择 / 子窗口_复制参数确认 / 子窗口_日志浏览

每个子窗口内组�?确认"�?取消"两个标准按钮构件,分别绑定对应 Click 脚本。子窗口尺寸建议 400×180(确认�?�?400×240(选择�?�?
...
| 打开窗口 | `!SwitchWindow("画面2")` (不存�? | `!CloseAllWindow("画面2_单元详情")` |
...
凡涉�?�?个单元相同操�?的脚�?本文件采用以下两种写法之一:
1. **显式展开**(用于必须同时操作8单元的场�?如脚�?/7/8/52):直接�?段If块或8条赋�?
2. **�?号为�?*(用于�?SelectedUnit 选择单单元操作的场景,如脚�?3/14/19~25/50/51):给出1号完整代�?+ 2~8号扩展说�?替换单元�?

这是因为 McgsPro **不支�?Execute 动态构造变量名**,无法用循�?字符串拼接访�?U1_XXX~U8_XXX。组态时必须�?8 套代码全部粘贴到对应�?8 个按�?Click 事件中�?
