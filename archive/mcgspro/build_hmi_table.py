...
hmi_vars = [
    ...
    ("VW304", "上缸配液子流程状态"),
    ("VW400", "已完成下缸换水次数（v2.3.2由VW306迁入，避让手动命令位）"),
    ("VW288", "S4转移计时T49的PT(100ms单位，v2.3.1迁入)"),
    # HMI parameters
    ("VD10", "目标浓度设定值（v2.0新增）"),
    ("VD14", "母液浓度设定值（v2.0新增）"),
    ("VD24", "实验时长目标设定值(min)"),
    ...
    ("VD178", "S5运行已用时长(s)"),
    ("VD244", "周期倒计时器B显示值(s,DINT)"),
    ("VD248", "TimerA已运行秒数(REAL,调试)"),
    ("VD256", "周期倒计时器A显示值(s,DINT)"),
    ("VD252", "TimerB已运行秒数(REAL,调试)"),
    ("VD308", "阀A关阀快照（HMI可查看）"),
    ("VD316", "目标进水量(L)"),
    ("VD440", "累计加药量(uL)"),
    ("VD444", "S4等待时长内部变量(s)"),
    ...
    ("V302.3", "阀B关到位反馈超时"),
    ("V302.4", "阀B关到位但仍有流"),
    ("V302.5", "阀C四态诊断异常（v10.4废弃,不再置位）"),
    ("V302.6", "阀C开到位反馈超时"),
    ("V302.7", "阀C开到位但无流（v10.4废弃）"),
    ("V303.0", "阀C关到位反馈超时"),
    ("V303.1", "阀C关到位但仍有流（v10.4废弃）"),
    ("V303.2", "S4等待超时报警"),
    ("V303.4", "注射泵通讯/动作异常"),
    ("V303.5", "RTC时钟丢失"),
    ("V303.6", "单轮换水周期超时"),
